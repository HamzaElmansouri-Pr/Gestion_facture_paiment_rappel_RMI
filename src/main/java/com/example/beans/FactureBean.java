...
public class FactureBean implements Serializable {
    ...
    public void ajouterLigne() {
        ...
        nouvelleLigne.setPrix(article.getPrix());
    ...
    public double getMontantTotal() {
        if (facture.getLignes() == null) return 0.0;
    ...
    public String enregistrer() {
        if (facture.getLignes() == null || facture.getLignes().isEmpty()) {
            return null; // Don't save if no lines
        }
        
        // Set the current date
        facture.setDateFacture(new java.util.Date());
        facture.setEtat(Etat.NON_PAYEE); // Set default status
